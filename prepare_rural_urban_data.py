...
# Define files for Urban and Rural
chains = {
    "rural": [
...
def process_file_raw(args):
    ...
    temp_path = os.path.join(scratch_dir, f"temp_{chain_name}_raw_{filename}")
...
def process_chain_data(chain_name, file_list, product_mch5_map):
    print(f"\n--- Extracting data for chain: {chain_name.upper()} ---")
    t_start = time.time()
    ...
    print(f"Total rows extracted for {chain_name}: {len(all_rows):,}")
    if not all_rows:
        print(f"No data extracted for {chain_name}.")
        return

    # Compress data using dictionary encoding for articles and IDs
    print(f"Compressing data for {chain_name}...")
    t_comp = time.time()
    ...
    print(f"  Compressed {len(compressed_rows):,} rows in {time.time()-t_comp:.2f}s")
    print(f"  Saved JSON to: {out_path} ({os.path.getsize(out_path)/(1024*1024):.2f} MB)")
...
def main():
    ...
    for chain_name, file_list in chains.items():
        process_chain_data(chain_name, file_list, product_mch5_map)
        
    print(f"\nAll chains data processed! Total time: {time.time()-t_total_start:.2f}s")
