...
def load_master_articles():
    print("Loading master articles from XLSB...", flush=True)
    t0 = time.time()
    mapping = {}
    try:
...
def process_chain(chain_name, product_mch5_map, workers):
    ...
    # Write Data Rows (with prints)
    for row_idx_0based, row in enumerate(all_data_rows):
        if row_idx_0based > 0 and row_idx_0based % 20000 == 0:
            print(f"  Writing row {row_idx_0based:,} / {total_rows:,}...", flush=True)
    ...
    print(f"  All {total_rows:,} rows appended. Applying column widths and saving workbook...", flush=True)
    # Apply column widths
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python process_chain_runner.py <chain_name>")
        sys.exit(1)
        
    chain_name = sys.argv[1].lower()
    if chain_name not in chains:
        print(f"Error: Unknown chain '{chain_name}'. Choose from 'win', 'rural', 'urban'")
        sys.exit(1)
