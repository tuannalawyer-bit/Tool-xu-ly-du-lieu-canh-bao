@echo off
chcp 65001 >nul
title Tool Chi Tiet - Phan Tich Ton Kho

echo ========================================================
echo  Tool Chi Tiet Phan Tich Ton Kho (Chính thức)
echo ========================================================
echo.
...
"%PYTHON_CMD%" -c "import openpyxl, pyxlsb, tkinter" >nul 2>&1
if errorlevel 1 (
    ...
    if exist "%~dp0Tool_PhanTichTonKho_Portable\libs" (
        ...
        "%PYTHON_CMD%" -m pip install openpyxl pyxlsb --quiet --timeout 5
        if errorlevel 1 (
...
"%PYTHON_CMD%" "%~dp0tool_chi_tiet.py"
